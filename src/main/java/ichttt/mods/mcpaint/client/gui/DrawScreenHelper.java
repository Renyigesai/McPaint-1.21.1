...
import ichttt.mods.mcpaint.networking.DrawAbortPayload;
...
public class DrawScreenHelper {
    ...
    public void onClose() {
        if (!noRevert) {
            PacketDistributor.sendToServer(new DrawAbortPayload(pos));
        }
    }
    ...
    public void renderImage(GuiGraphics graphics, int startLeft, int startTop, int[][] toDraw) {
        ...
        RenderUtil.renderInGui(matrix, startLeft, startTop, this.currentState.scaleFactor, buffer, toDraw);
